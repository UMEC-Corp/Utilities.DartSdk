//
// AUTO-GENERATED FILE, DO NOT MODIFY!
//
// @dart=2.18

// ignore_for_file: unused_element, unused_import
// ignore_for_file: always_put_required_named_parameters_first
// ignore_for_file: constant_identifier_names
// ignore_for_file: lines_longer_than_80_chars

import 'package:utilities_dart_sdk_client/api.dart';
import 'package:test/test.dart';

// tests for UtilitiesWebApiProtosUpdateContactsRequest
void main() {
  // final instance = UtilitiesWebApiProtosUpdateContactsRequest();

  group('test UtilitiesWebApiProtosUpdateContactsRequest', () {
    // The ID of the virtual device.
    // String unitId
    test('to test the property `unitId`', () async {
      // TODO
    });

    // The list of contacts to update.
    // List<UtilitiesWebApiProtosContact> contacts (default value: const [])
    test('to test the property `contacts`', () async {
      // TODO
    });
  });
}
